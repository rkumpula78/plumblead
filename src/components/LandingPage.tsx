...
const PhoneMockup: React.FC = () => {
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ display:'inline-flex',alignItems:'center',gap:8,background:'rgba(245,166,35,0.1)',border:'1px solid rgba(245,166,35,0.3)',borderRadius:20,padding:'4px 14px',fontSize:11,color:'#F5A623',fontWeight:700,letterSpacing:1,marginBottom:20 }}>LIVE LEAD RESPONSE</div>
      <div style={{ width:280,background:'#1A1A1A',border:'2px solid #333',borderRadius:20,overflow:'hidden',boxShadow:'0 0 60px rgba(245,166,35,0.15)',margin:'0 auto' }}>
        ...
        <div style={{ padding:16,display:'flex',flexDirection:'column',gap:8,minHeight:280 }}>
          <div style={{ background:'#2A2A2A',color:'#DDD',borderRadius:'4px 12px 12px 12px',padding:'10px 14px',fontSize:13,lineHeight:1.4,maxWidth:200 }}>Hi, my water heater is leaking. How much to replace it?</div>
          ...
          {phase===1 && (
            <div style={{ display:'flex',gap:4,alignItems:'center',padding:'10px 14px',background:'#2A2A2A',borderRadius:'4px 12px 12px 12px',width:'fit-content' }}>
              {[0,200,400].map((d,i)=>(<div key={i} style={{ width:6,height:6,background:'#5C5A53',borderRadius:'50%',animation:`bounce 1.2s ${d}ms infinite` }}/>))}
            </div>
          )}
          {phase>=2 && (
            <>
              <div style={{ background:'#F5A623',color:'#000',borderRadius:'12px 4px 12px 12px',padding:'10px 14px',fontSize:13,lineHeight:1.4,maxWidth:210,alignSelf:'flex-end',fontWeight:600 }}>Replacement typically runs $1,200–$1,800. Can we schedule a same-day assessment?</div>
              <div style={{ fontSize:10,color:'#5C5A53',textAlign:'right',alignSelf:'flex-end' }}>PlumbLead.ai – 2:14 PM</div>
            </>
          )}
          {phase>=3 && (
            <div style={{ background:'#2A2A2A',color:'#DDD',borderRadius:'4px 12px 12px 12px',padding:'10px 14px',fontSize:13,lineHeight:1.4,maxWidth:180 }}>Yes please! When can you come?</div>
          )}
        </div>
        ...
        <div style={{ textAlign:'center',fontSize:11,color:'#F5A623',fontWeight:700,letterSpacing:1,padding:'10px 16px 16px',background:'rgba(245,166,35,0.05)' }}>Response in 47 seconds</div>
      </div>
...
const QuoteTool: React.FC<{ lang: 'en' | 'es' }> = ({ lang }) => {
  ...
  const [form, setForm] = useState<QuoteFormData>({ serviceType:'',details:'',location:'',name:'',phone:'',email:'',language:lang });
  ...
  const inputStyle: React.CSSProperties = { width:'100%',padding:'14px 16px',fontSize:15,border:'2px solid #E8E6DF',background:'#FFF',fontFamily:'DM Sans, sans-serif',outline:'none',transition:'border-color 0.2s',color:'#0D0D0D' };
    ...
    <div style={{ maxWidth:680,margin:'0 auto' }}>
        ...
        <div style={{ display:'flex',gap:0,marginBottom:40 }}>
          {[1,2,3].map(s => (
            <div key={s} style={{ flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:8 }}>
              ...
              <div style={{ fontSize:12,fontWeight:700,color:step>=s?'#C4841A':'#9E9B91',textTransform:'uppercase',letterSpacing:1 }}>{s===1?t.step1:s===2?t.step2:t.step3}</div>
            </div>
          ))}
        </div>
      )}
      {step===1 && (
        <div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12 }}>
            {services.map((svc,i) => (
              <button key={i} onClick={()=>{ updateForm('serviceType',SERVICES[i]); setStep(2); }} style={{ padding:'16px 20px',textAlign:'left',fontSize:14,fontWeight:600,border:form.serviceType===SERVICES[i]?'2px solid #F5A623':'2px solid #E8E6DF',background:form.serviceType===SERVICES[i]?'#FFF3D6':'#FFF',cursor:'pointer',fontFamily:'DM Sans, sans-serif',color:'#0D0D0D' }}>{svc}</button>
            ))}
          </div>
        </div>
      )}
      {step===2 && (
        <div>
          ...
          <div style={{ marginBottom:20 }}><label style={labelStyle}>Job Details</label><textarea value={form.details} onChange={e=>updateForm('details',e.target.value)} placeholder={t.detailsPlaceholder} rows={5} style={{ ...inputStyle,resize:'vertical' }}/></div>
          <div style={{ marginBottom:32 }}><label style={labelStyle}>Location</label><input type="text" value={form.location} onChange={e=>updateForm('location',e.target.value)} placeholder={t.locationPlaceholder} style={inputStyle}/></div>
      ...
      {step===3 && (
        <div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16 }}>
            <div><label style={labelStyle}>Name</label><input type="text" value={form.name} onChange={e=>updateForm('name',e.target.value)} placeholder={t.namePlaceholder} style={inputStyle}/></div>
            <div><label style={labelStyle}>Phone</label><input type="tel" value={form.phone} onChange={e=>updateForm('phone',e.target.value)} placeholder={t.phonePlaceholder} style={inputStyle}/></div>
          </div>
          <div style={{ marginBottom:20 }}><label style={labelStyle}>Email</label><input type="email" value={form.email} onChange={e=>updateForm('email',e.target.value)} placeholder={t.emailPlaceholder} style={{ ...inputStyle,width:'100%' }}/></div>
          <div style={{ display:'flex',alignItems:'flex-start',gap:10,marginBottom:24,padding:'12px 16px',background:'#F5F4F0',border:'1px solid #E8E6DF' }}>
            ...
            <label htmlFor="sms-consent" style={{ fontSize:12,color:'#5C5A53',lineHeight:1.5,cursor:'pointer' }}>I agree to receive SMS updates about my service request from PlumbLead.ai. Message &amp; data rates may apply. Reply STOP to opt out.</label>
          </div>
          ...
          <div style={{ display:'flex',gap:12 }}>
            ...
            <button onClick={handleSubmit} disabled={!canSubmit} style={{ flex:1,padding:'14px 24px',background:canSubmit?'#F5A623':'#E8E6DF',border:'none',fontWeight:700,fontSize:15,cursor:canSubmit?'pointer':'not-allowed',fontFamily:'DM Sans, sans-serif',color:canSubmit?'#000':'#9E9B91' }}>{loading?t.generating:t.getQuote}</button>
          </div>
        ...
        <div>
          ...
          {result.suggestedNextSteps?.length>0&&(
            <div style={{ background:'#F5F4F0',padding:'24px 28px',marginBottom:16 }}>
              ...
              {result.suggestedNextSteps.map((s,i)=>(<div key={i} style={{ display:'flex',gap:12,alignItems:'flex-start',marginBottom:12 }}><div style={{ width:24,height:24,background:'#F5A623',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,flexShrink:0 }}>{i+1}</div><p style={{ fontSize:14,color:'#5C5A53',lineHeight:1.5 }}>{s}</p></div>))}
            </div>
          )}
          {result.leadScore&&(<div style={{ display:'flex',gap:12,alignItems:'center',marginBottom:16,padding:'12px 16px',border:'1px solid #E8E6DF' }}><span style={{ fontSize:12,fontWeight:700,color:'#9E9B91',textTransform:'uppercase',letterSpacing:1 }}>{t.leadScoreLabel}:</span><span style={{ background:'#F5A623',color:'#000',padding:'3px 12px',fontSize:13,fontWeight:700 }}>{result.leadScore}</span></div>)}
          {result.crossSellOpportunities?.length>0&&(<div style={{ padding:'16px',border:'1px solid #E8E6DF',marginBottom:24 }}><div style={{ fontSize:11,textTransform:'uppercase',letterSpacing:2,color:'#C4841A',fontWeight:700,marginBottom:8 }}>{t.crossSellLabel}</div><div style={{ display:'flex',gap:8,flexWrap:'wrap' }}>{result.crossSellOpportunities.map((o,i)=>(<span key={i} style={{ background:'#FFF3D6',color:'#7A5810',padding:'4px 12px',fontSize:13,fontWeight:600 }}>{o}</span>))}</div></div>)}
          <button onClick={reset} style={{ width:'100%',padding:'14px',border:'2px solid #0D0D0D',background:'transparent',fontWeight:700,fontSize:15,cursor:'pointer',fontFamily:'DM Sans, sans-serif',color:'#0D0D0D' }}>{t.startOver}</button>
        </div>
      )}
    </div>
  );
    ...
    <>
      ...
      <nav style={{ position:'sticky',top:0,zIndex:100,background:'#0D0D0D',borderBottom:'3px solid #F5A623',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 60px',height:64 }}>
        ...
        <div style={{ display:'flex',gap:32,alignItems:'center' }}>
          {[['#how',t.nav.howIt],['#compare',t.nav.compare],['#pricing',t.nav.pricing],['#faq',t.nav.faq]].map(([href,label])=>(<a key={href} href={href} style={{ color:'#9E9B91',textDecoration:'none',fontSize:14,fontWeight:500 }}>{label}</a>))}
        </div>
      ...
      <section style={{ background:'#0D0D0D',display:'grid',gridTemplateColumns:'1fr 1fr',minHeight:620,borderBottom:'4px solid #F5A623' }}>
        <div style={{ padding:'80px 60px 80px 80px',display:'flex',flexDirection:'column',justifyContent:'center',borderRight:'2px solid #222' }}>
          ...
          <h1 style={{ fontFamily:'Bebas Neue, sans-serif',fontSize:88,lineHeight:0.92,color:'#FFF',textTransform:'uppercase',marginBottom:16 }}>
            {t.hero1}<br/>
            <span style={{ position:'relative',color:'#555' }}>{t.hero2}<span style={{ position:'absolute',left:0,right:0,top:'50%',height:4,background:'#D83030',transform:'translateY(-50%)',display:'block' }}/></span><br/>
            <span style={{ color:'#F5A623' }}>{t.hero3}</span><br/>{t.hero4}
          </h1>
          ...
          <div style={{ marginTop:40,display:'flex',gap:24,flexWrap:'wrap' }}>
            {t.trust.map((item,i)=>(<div key={i} style={{ display:'flex',alignItems:'center',gap:6,fontSize:13,color:'#9E9B91',fontWeight:500 }}><span style={{ color:'#F5A623' }}>✓</span> {item}</div>))}
          </div>
        </div>
      ...
      <div style={{ background:'#F5A623',display:'grid',gridTemplateColumns:'repeat(3,1fr)',borderBottom:'4px solid #0D0D0D' }}>
        {[{num:'78%',label:'Jobs Won by First Responder',sub:'Speed isn\'t just nice — it\'s the job'},{num:'2hrs',label:'Average Response Time',sub:'That\'s how slow your competition is'},{num:'$3,200',label:'Average Job Value Lost',sub:'Per missed lead, every single time'}].map((s,i)=>(
      ...
      <section id="compare" style={{ background:'#0D0D0D',padding:'80px' }}>
        <div style={{ fontSize:11,textTransform:'uppercase',letterSpacing:2,fontWeight:700,color:'#F5A623',marginBottom:16,display:'flex',alignItems:'center',gap:8 }}><span style={{ display:'block',width:24,height:2,background:'#F5A623' }}/> Head-to-Head</div>
      ...
      <section id="how" style={{ background:'#F5F4F0',padding:'80px' }}>
        <div style={{ fontSize:11,textTransform:'uppercase',letterSpacing:2,fontWeight:700,color:'#C4841A',marginBottom:16,display:'flex',alignItems:'center',gap:8 }}><span style={{ display:'block',width:24,height:2,background:'#F5A623' }}/> The Process</div>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)' }}>
          {[{num:'01',icon:'📲',title:'Lead Lands',body:'A homeowner submits a request or calls after-hours. PlumbLead captures it instantly — no missed calls, no lost form submissions.'},{num:'02',icon:'⚡',title:'AI Qualifies',body:'Our AI assesses urgency, generates a ballpark estimate, and responds to the homeowner in under 60 seconds. Bilingual. Professional. On-brand.'},{num:'03',icon:'💰',title:'You Close',body:'The qualified lead — with urgency score, quote data, and full context — lands in your CRM or phone. You show up prepared to close.'}].map((step,i)=>(
      ...
      <div style={{ background:'#0D0D0D',padding:'80px' }}>
        <div style={{ maxWidth:800,margin:'0 auto',textAlign:'center' }}>
          ...
          <div style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:16 }}>
            ...
            <div style={{ textAlign:'left' }}><div style={{ fontWeight:700,color:'#FFF',fontSize:16 }}>Mike R.</div><div style={{ fontSize:13,color:'#5C5A53' }}>Owner, Reliable Plumbing Services — Phoenix, AZ</div></div>
          </div>
        </div>
      </div>

      <section ref={quoteRef} id="quote" style={{ background:'#FFF',padding:'80px' }}>
        <div style={{ fontSize:11,textTransform:'uppercase',letterSpacing:2,fontWeight:700,color:'#C4841A',marginBottom:16,display:'flex',alignItems:'center',gap:8 }}><span style={{ display:'block',width:24,height:2,background:'#F5A623' }}/> {t.quoteSection}</div>
