...
const ProMaxDemo: React.FC = () => {
    ...
    return (
      <div style={{ ...s, minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ maxWidth: 480, width: '100%', background: '#fff', borderRadius: 16, padding: 40, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 15, color: '#64748b', lineHeight: 1.6, marginBottom: 24 }}>Thanks {name.split(' ')[0]}! Someone from <strong>ProMax Water Heaters & Plumbing</strong> will reach out within 60 seconds.</p>
  ...
  return (
    <div style={{ ...s, minHeight: '100vh', background: '#f8fafc' }}>
      ...
      {/* Hero */}
      <div style={{ background: `linear-gradient(135deg, ${CLIENT_COLOR} 0%, #0f3460 100%)`, color: '#fff', padding: '40px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.8)', maxWidth: 460, margin: '0 auto', lineHeight: 1.5 }}>Licensed journeymen plumbers. Response in under 60 seconds.</p>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 24px' }}>
          ...
          {step === 1 && (
            <div style={{ opacity: mounted ? 1 : 0, transition: 'opacity 0.3s' }}>
              ...
              <p style={{ fontSize: 14, color: '#64748b', marginBottom: 24 }}>Select the service and we'll generate an instant estimate.</p>
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginBottom: 20 }}>
                ...
                <textarea value={details} onChange={e => setDetails(e.target.value)} placeholder="e.g. 40-gallon gas water heater, 12 years old, pilot light keeps going out. Located in Monroe, WA." rows={4} style={{ width: '100%', padding: '12px', border: '2px solid #e2e8f0', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', resize: 'vertical', outline: 'none', boxSizing: 'border-box' as const }} />
            ...
            <div>
              ...
              {result.crossSellOpportunities && result.crossSellOpportunities.length > 0 && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const, marginBottom: 20 }}>
                  {result.crossSellOpportunities.map((o, i) => (<span key={i} style={{ fontSize: 11, background: '#eff6ff', color: CLIENT_COLOR, padding: '3px 10px', borderRadius: 20, fontWeight: 700 }}>{o}</span>))}
                </div>
              )}
              <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '12px', background: '#f8fafc', borderRadius: 8 }}>
                    ...
                    <label htmlFor="consent" style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5, cursor: 'pointer' }}>I agree to receive SMS updates from ProMax Water Heaters & Plumbing. Reply STOP to opt out.</label>
                  </div>
      ...
      <div style={{ background: CLIENT_COLOR, color: '#fff', padding: '28px 24px', textAlign: 'center', marginTop: 40 }}>
        ...
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', marginBottom: 10 }}>17461 147th St SE #6A, Monroe, WA 98272</div>
