...
// Cap slug at 30 chars, trim trailing dashes
...
export default function AdminOnboard() {
  ...
  async function handleScrape() {
    ...
    try {
      ...
      setScrapeError(e.message || 'Scrape failed. Check the URL and try again.');
    } finally {
      setScraping(false);
    }
  }
      ...
      <div style={s.card}>
        <div style={s.success}>
          ...
          <p style={{ color: '#374151', fontSize: 15, margin: '8px 0 24px' }}>
            Contractor added to Postgres. Copy these to complete setup.
          </p>
        </div>
        ...
        <div style={s.section}>
          ...
          <p style={{ fontSize: 12, color: '#64748b', marginBottom: 20 }}>
            Add to <code>ACCESS_CODES</code> in Dashboard.tsx:<br />
            <code style={{ background: '#f1f5f9', padding: '2px 6px', display: 'inline-block', marginTop: 4 }}>
              '{accessCode}': {'{ '}clientId: '{saved.clientId}', label: '{saved.companyName}'{' }'}
            </code>
          </p>
        </div>
        ...
        <div style={s.section}>
          ...
          <p style={{ fontSize: 12, color: '#64748b' }}>Demo page not yet created — build with demo page builder.</p>
        </div>

        <button style={{ ...s.btn, marginTop: 24 }}
          onClick={() => { setSaved(null); setForm(BLANK); setScrapeUrl(''); }}>
        ...
        {tab === 'scrape' && (
          <div>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 12, textAlign: 'center' }}>
              After extraction you'll review and confirm in the manual form.
            </p>
          </div>
        )}
          ...
          <div>
            ...
            <div style={s.row2}>
              ...
              <div>
                ...
                <p style={s.hint}>Auto-generated from name. Edit if needed. Max 30 chars.</p>
              </div>
            </div>
            ...
            <div style={s.section}>
              ...
              <div style={s.row3}>
                ...
                <div>
                  ...
                  <input style={s.input} placeholder="85383, 85374..."
                    value={form.zipCodes} onChange={e => setField('zipCodes', e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div style={s.section}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                ...
                <label htmlFor="bilingual" style={{ ...s.label, marginBottom: 0, fontWeight: 400, cursor: 'pointer' }}>
            ...
            <div style={s.section}>
              ...
              <div style={s.servGrid}>
                {ALL_SERVICES.map(svc => (
                  <div key={svc.key} style={servStyle(form.services.includes(svc.key))}
            ...
            {form.clientId && (
              <div style={{ ...s.section, background: '#f8fafc', padding: 16, border: '1px solid #e2e8f0' }}>
                ...
                <p style={{ fontSize: 13, color: '#374151', margin: '4px 0' }}>
                  <strong>Dashboard access code:</strong>{' '}
                  <code style={{ background: '#e2e8f0', padding: '2px 6px' }}>{accessCode}</code>
                </p>
                <p style={{ fontSize: 13, color: '#374151', margin: '4px 0' }}>
                  <strong>Demo page:</strong>{' '}
                  <code style={{ background: '#e2e8f0', padding: '2px 6px' }}>plumblead.ai/demo/{form.clientId}</code>
                </p>
                <p style={{ fontSize: 13, color: '#374151', margin: '4px 0' }}>
                  <strong>Embed src:</strong>{' '}
                  <code style={{ background: '#e2e8f0', padding: '2px 6px' }}>...quote?widget=1&client={form.clientId}</code>
                </p>
              </div>
            )}
